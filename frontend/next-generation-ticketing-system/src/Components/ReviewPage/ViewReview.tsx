...
function ViewReview() {
    ...
    const handleLeftSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        setLeftSelectedOption(event.target.value);
    };

    const handleRightSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const handleLeftFindButtonClick = () => {
        let user = leftOptions.find((option) => option.user.userId === Number(leftSelectedOption))?.user;
        if(user){
    ...
    const handleRightFindButtonClick = () => {
        let bus = rightOptions.find((option) => option.busNo === rightSelectedOption);
        if(bus){
    ...
    return (
        <div className="flex justify-center h-screen background_image_homepage body_design">
            <div className="flex w-1/2 p-6 mt-16  gap-x-10">
                <div id="left" className=" w-full p-4 bg-black bg-opacity-75 text-white font-bold text-lg rounded-2xl">
                    <div className="flex items-center mb-4">
                        ...
                        <button
                            ...
                            className={`bg-blue-500 text-white p-2 ml-2 rounded ${isLeftFindButtonDisabled ? "opacity-50 cursor-not-allowed" : ""}`}
                        >
                            Find
                        </button>
                    </div>
                    {leftReview.map((review, index) => (
                        <div key={index} className="flex flex-col mb-2 border-2 border-teal-500 p-2 rounded">
                            <div>Date: {formatTime(review.reviewDate)}</div>
                            <div className="flex justify-between">
                                <div className="flex">
                                    <div>Reviewer:</div>
                                    <div className="ml-2 text-blue-500">{review.reviewer.email}</div>
                                </div>
                                <div className="flex">
                                    <div>Rating:</div>
                                    <div className="ml-2" style={{ color: getReviewPointColor(review.reviewPoint) }}>{review.reviewPoint}</div>
                                </div>
                            </div>
                            <div>
                                <div>Comment:</div>
                ...
                <div id="right" className=" w-full p-4 bg-black bg-opacity-75 text-white font-bold text-lg rounded-2xl">
                    <div className="flex items-center mb-4">
                        <select
                        ...
                        <button
                            ...
                            className={`bg-blue-500 text-white p-2 ml-2 rounded ${isRightFindButtonDisabled ? "opacity-50 cursor-not-allowed" : ""}`}
                        >
                            Find
                        </button>
                    </div>
